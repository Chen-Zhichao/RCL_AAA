...
def gradual_adaptive_instance_normalization(content_feat, style_feat, mean_factor, std_factor):
    assert (content_feat.size()[:2] == style_feat.size()[:2])
    size = content_feat.size()
    style_mean, style_std = calc_mean_std(style_feat)
    content_mean, content_std = calc_mean_std(content_feat)

    out = np.power((style_std.expand(size) / content_std.expand(size)), std_factor) * \
        (content_feat - mean_factor * content_mean.expand(size)) + \
            mean_factor * style_mean.expand(size)
    return out
