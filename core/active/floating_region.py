...
class FloatingRegionScore(nn.Module):
    ...
    def forward(self, logit, now_iteration):
        """
        return:
        ...
        # print("Now the confidence weight is : {}".format(confidence_weight))
        pixel_entropy = torch.sum(-p * torch.log(p + 1e-6), dim=0).unsqueeze(dim=0).unsqueeze(dim=0) / math.log(19) - confidence_weight * p.max(axis=0)[0].unsqueeze(dim=0).unsqueeze(dim=0) + 1# [1, 1, h, w], latter modified by czc
