...
def PixelSelection(cfg, feature_extractor, classifier, tgt_epoch_loader):
    ...
    with torch.no_grad():
        for tgt_data in tqdm(tgt_epoch_loader, position=0, mininterval=10):
            ...
            # print("Here is origin_mask size:{}".format(origin_mask.size()))     # added by czc
            ...
            for i in range(len(origin_mask)):
                print("Here is length of origin_mask: {}".format(len(origin_mask)))
...
def RegionSelection(cfg, feature_extractor, classifier, tgt_epoch_loader, now_iteration):
    ...
    with torch.no_grad():
        for tgt_data in tqdm(tgt_epoch_loader, position=0, mininterval=10):
            ...
            for i in range(len(origin_mask)):
                ...
                output = tgt_out[i:i + 1, :, :, :]
                # print("logit size before interpolate is: {}".format(output.size()))     # added by czc
                output = F.interpolate(output, size=size, mode='bilinear', align_corners=True)
                # print("logit size after interpolate is: {}".format(output.size()))      # added by czc
                score, purity, entropy = floating_region_score(output, now_iteration=now_iteration)
