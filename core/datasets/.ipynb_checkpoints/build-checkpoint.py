...
def build_segmentation_transform(cfg, mode, is_source):
    if mode == "train":
...
def build_style_transfer_transform(cfg, is_source):
    w, h = cfg.INPUT.SOURCE_INPUT_SIZE_TRAIN if is_source else cfg.INPUT.TARGET_INPUT_SIZE_TRAIN
    # trans = transform.Compose([
    #     transform.Resize((h, w), resize_label=False),
    #     transform.RandomCrop(size=(h, w), pad_if_needed=True),
...
def build_segmentation_dataset(cfg, mode='train', is_source=True, epochwise=False, empty=False):
    assert mode in ['train', 'val', 'test', 'active']
    transform = build_segmentation_transform(cfg, mode, is_source)
    print(transform)
    iters=None
...
def build_style_transfer_dataset(cfg, mode='train', is_source=True, epochwise=False, empty=False):
    transform = build_style_transfer_transform(cfg, is_source)
    print(transform)
    iters=None
    if is_source:
        dataset = DatasetCatalog.get(cfg.DATASETS.SOURCE_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES,
